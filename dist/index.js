...
var NotInitializedError = class extends ChioBridgeError {
  constructor(message = "bridge not initialized in daemon mode") {
    super("not_initialized", message);
    this.name = "NotInitializedError";
  }
};
var PolicyParseError = class extends ChioBridgeError {
  path;
...
var CapabilityDeniedError = class extends ChioBridgeError {
  guard;
  constructor(message, guard, detail) {
    super("capability_denied", message, detail);
    this.name = "CapabilityDeniedError";
    this.guard = guard;
  }
};
var DaemonUnreachableError = class extends ChioBridgeError {
  url;
  constructor(message, url, detail) {
    super("daemon_unreachable", message, detail);
    this.name = "DaemonUnreachableError";
    this.url = url;
  }
};
var CliError = class extends ChioBridgeError {
  stderr;
  exitCode;
...
function hasOnPath(name) {
  try {
    ...
    return probe.status !== null && probe.error === void 0;
  } catch {
    return false;
  }
}
var ChioCli = class {
  ...
  async runJson(args, options = {}) {
    ...
    if (text.length === 0) {
      throw new CliError(`chio ${args.join(" ")} produced no JSON output`, result.exitCode, result.stderr);
    }
    try {
      return JSON.parse(extractLastJson(text));
    } catch (cause) {
      throw new CliError(`chio ${args.join(" ")} produced unparseable JSON`, result.exitCode, `${result.stderr}
...
var QueryError = class extends ChioError {
  status;
...
var TransportError = class extends ChioError {
...
var DID_PREFIXES = ["did:chio:", "did:arc:"];
function isChioDid(s) {
  if (!s)
    return false;
  return DID_PREFIXES.some((p) => s.startsWith(p));
}
function didSuffix(s) {
  if (!s)
    return void 0;
...
var DaemonClient = class {
  ...
  async trust(method, path, body, headers = {}) {
    ...
    try {
      ...
      throw new DaemonUnreachableError(`trust plane unreachable at ${url}: ${cause.message}`, this.trustUrl, cause);
    }
    const raw = await response.text();
    let data;
    try {
      data = raw.length === 0 ? void 0 : JSON.parse(raw);
    } catch {
      data = raw;
    }
...
async function issueCapability(daemon, input) {
  if (!daemon) {
    throw new NotInitializedError("issueCapability() requires daemon mode; construct via ChioBridge.fromDaemon({...})");
  }
  let subjectPublicKey = input.subjectPublicKey;
  let subjectPrivateKeyHex;
  if (!subjectPublicKey) {
    const suffix = typeof input.subject === "string" ? didSuffix(input.subject) : void 0;
  ...
  if (input.delegatable !== void 0)
    body.delegatable = input.delegatable;
  if (input.runtimeAttestation !== void 0) {
    body.runtimeAttestation = input.runtimeAttestation;
  }
  const res = await daemon.trust("POST", "/v1/capabilities/issue", body);
  if (!res.ok) {
    const err = res.data?.error ?? res.raw.slice(0, 300);
    throw new CapabilityDeniedError(`capability issuance failed: HTTP ${res.status}: ${err}`);
  }
  const data = res.data;
  const token = data.capability ?? data;
  const issued = { ...token, subjectPublicKey };
  if (subjectPrivateKeyHex)
    issued.subjectPrivateKeyHex = subjectPrivateKeyHex;
  return issued;
}
async function attenuateCapability(daemon, capabilityId, delta) {
  if (!daemon) {
...
function generateEd25519KeyPairHex() {
...
async function issueCapabilityViaHttp(trustUrl, token, input) {
  let subjectPublicKey = input.subjectPublicKey;
  let subjectPrivateKeyHex;
  if (!subjectPublicKey) {
    const generated = generateEd25519KeyPairHex();
  ...
  if (!res.ok) {
    throw new CapabilityDeniedError(`capability issuance failed: HTTP ${res.status}: ${raw.slice(0, 300)}`);
  }
  let parsed;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new ChioBridgeError("capability_issue_parse", `trust plane returned non-JSON issuance response: ${raw.slice(0, 300)}`);
  }
  const data = parsed;
  const tokenOut = data.capability ?? data;
  const issued = { ...tokenOut, subjectPublicKey };
  if (subjectPrivateKeyHex)
    issued.subjectPrivateKeyHex = subjectPrivateKeyHex;
  return issued;
}
...
function normalizeVerdict(out) {
  ...
  if (typeof raw === "string") {
    ...
    const obj = raw;
    const explicitTag = obj.type ?? obj.kind ?? obj.verdict;
    const tag = explicitTag ?? Object.keys(obj)[0];
    const lowered = tag?.toLowerCase();
    if (lowered === "allow")
      decision = "allow";
    else if (lowered === "cancelled")
      decision = "cancelled";
    else
      decision = "deny";
    ...
    if (tag && !explicitTag) {
      ...
      if (inner && typeof inner === "object") {
        ...
        if (typeof innerObj.reason === "string")
          reason ??= innerObj.reason;
        if (typeof innerObj.guard === "string")
          guard ??= innerObj.guard;
      }
    }
  } else {
    decision = "deny";
...
async function discoverMcpServers(daemon) {
  ...
  try {
    ...
    throw new ChioBridgeError("mcp_admin_unreachable", `MCP edge admin endpoint unreachable at ${url}: ${cause.message}`, cause);
  }
  if (!response.ok) {
    throw new ChioBridgeError("mcp_admin_failed", `MCP edge /admin/sessions returned HTTP ${response.status}`);
  }
  ...
  return sessions.map((s) => {
    ...
    if (s.transport === "http" || s.transport === "stdio")
      info.transport = s.transport;
    return info;
  });
}
async function wrapMcp(cli, cmd, options = {}) {
  if (cmd.length === 0) {
  ...
  args.push("--", ...cmd);
  const child = spawn2(cli.binary, args, {
    stdio: ["ignore", "pipe", "pipe"],
    env: options.env ?? process.env
  });
  const timeoutMs = options.readinessTimeoutMs ?? 1e4;
  let url;
  let resolvedListen;
  try {
...
function deriveServerId(cmdHead) {
  const h = createHash2("sha256").update(cmdHead).digest("hex").slice(0, 16);
  return `chio-wrap-${h}`;
}
async function waitForReady(child, timeoutMs) {
  return new Promise((resolve4, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      settled = true;
      detach();
      reject(new ChioBridgeError("mcp_wrap_timeout", `chio mcp serve-http did not signal readiness within ${timeoutMs}ms: ${buffer.slice(-400)}`));
    }, timeoutMs);
    const onData = (chunk) => {
      buffer += typeof chunk === "string" ? chunk : chunk.toString("utf8");
      const exact = buffer.match(/remote MCP edge listening on (https?:\/\/([^\s/]+)(?:\/\S*)?)/i);
      if (exact && !settled) {
        settled = true;
        clearTimeout(timer);
        detach();
    ...
    child.once("exit", (code) => {
      ...
      clearTimeout(timer);
      detach();
      reject(new ChioBridgeError("mcp_wrap_exit", `chio mcp serve-http exited before readiness (code ${code}): ${buffer.slice(-400)}`));
    });
  });
}
...
async function createPassport(daemon, cli, opts = {}) {
  if (!cli) {
    throw new NotInitializedError("createPassport() requires the chio CLI (chio binary on $PATH or CHIO_BIN)");
  }
  const receiptDbPath = opts.receiptDbPath ?? process.env.CHIO_RECEIPT_DB ?? process.env.CHIO_HARNESS_RECEIPT_DB ?? deriveHarnessReceiptDb();
  if (!receiptDbPath) {
    throw new ChioBridgeError("passport_create_failed", "createPassport() requires a receipt database path (set CreatePassportOptions.receiptDbPath, CHIO_RECEIPT_DB, or provide DaemonOptions.receiptDbPath) \u2014 chio passport create reads receipts from that DB to build the attested reputation credential.");
  }
  ...
  try {
    ...
    if (createResult.exitCode !== 0) {
      throw new ChioBridgeError("passport_create_failed", `chio passport create exited ${createResult.exitCode}: ${createResult.stderr.trim() || createResult.stdout.trim()}`);
    }
    let passportJson;
    try {
      passportJson = JSON.parse(await readFile(passportPath, "utf8"));
    } catch (cause) {
      throw new ChioBridgeError("passport_invalid", `chio passport create did not emit parseable JSON at ${passportPath}: ${cause.message}`, cause);
    }
    if (typeof passportJson.subject !== "string" || !isChioDid(passportJson.subject)) {
      throw new ChioBridgeError("passport_invalid", `chio passport create produced a passport with non-did:chio subject: ${passportJson.subject ?? "<missing>"}`);
    }
    let record;
    if (daemon) {
      const envelope = { passport: passportJson };
      const res = await daemon.trust("POST", "/v1/passport/statuses", envelope);
      if (!res.ok) {
        const detail = res.data?.error ?? res.raw.slice(0, 300);
        throw new ChioBridgeError("passport_publish_failed", `passport publish failed: HTTP ${res.status}: ${detail}`);
      }
      record = res.data;
      if (!record.subject || !isChioDid(record.subject)) {
        throw new ChioBridgeError("passport_invalid", `trust plane returned invalid lifecycle record (expected did:chio: subject, got ${record.subject ?? "<empty>"})`);
      }
    }
    ...
    if (record) {
      ...
      if (typeof record.publishedAt === "number") {
        passport.publishedAt = new Date(record.publishedAt * 1e3).toISOString();
      }
    }
    return passport;
  } finally {
...
async function verifyPassport(daemon, cli, input) {
  let did;
  let passportId;
  let file;
  ...
  if (file) {
    if (!cli) {
      throw new NotInitializedError("verifyPassport({ file }) requires the chio CLI (chio binary on $PATH or CHIO_BIN)");
    }
    const result = await cli.run(["passport", "verify", "--input", file]);
    return result.exitCode === 0;
  }
  if (did && !isChioDid(did))
    return false;
  if (daemon) {
    const listRes = await daemon.trust("GET", "/v1/passport/statuses");
    if (!listRes.ok)
      return false;
    const records = listRes.data.passports ?? [];
    const matches = records.filter((r) => {
      if (passportId && r.passportId === passportId)
        return true;
      if (did && r.subject === did)
        return true;
      return false;
    });
    if (matches.length === 0)
  ...
  if (cli) {
    throw new ChioBridgeError("verify_requires_file_or_daemon", "verifyPassport(did) requires daemon mode; for CLI-only verification pass { file: '/path/to/passport.json' } (the --output of createPassport / `chio passport create`).");
  }
  throw new NotInitializedError("verifyPassport() requires daemon or CLI client");
}
function deriveHarnessReceiptDb() {
...
async function writeFreshSigningSeed(seedPath) {
  ...
  await writeFile(seedPath, seedHex, { mode: 384 });
  void createPrivateKey2;
  void createPublicKey2;
  return { seedHex, subjectPublicKeyHex };
}
async function resolveSubjectPublicKey(cli, receiptDbPath, preferredHex) {
  try {
...
var MERGE_STRATEGIES = /* @__PURE__ */ new Set(["replace", "merge", "deep_merge"]);
var KNOWN_EXTENSION_FALLBACKS = {
...
async function loadPolicy(path) {
  ...
  try {
    ...
    throw new PolicyParseError(`failed to read policy at ${path}: ${cause.message}`, path, cause);
  }
...
function parsePolicy(text, path) {
  let doc;
  try {
    doc = import_yaml.default.parse(text);
  } catch (cause) {
    throw new PolicyParseError(`YAML parse error: ${cause.message}`, path, cause);
  }
  ...
  if (typeof version !== "string") {
    throw new PolicyParseError("policy is missing required `hushspec` version field", path);
  }
  if (version !== HUSHSPEC_SUPPORTED_VERSION) {
    throw new PolicyParseError(`unsupported hushspec version "${version}" (expected "${HUSHSPEC_SUPPORTED_VERSION}")`, path);
  }
  return obj;
}
async function lintPolicy(input) {
  const policy = typeof input === "string" ? await loadPolicy(input) : input;
  const errors = [];
  const warnings = [];
  ...
  for (const key of Object.keys(policyObj)) {
    if (!TOP_LEVEL_KEYS.has(key)) {
      errors.push({
        ...
        severity: "error",
        suggestion: "HushSpec 0.1.0 permits only: hushspec, name, description, extends, merge_strategy, rules, extensions, metadata"
      });
    }
  }
    ...
    for (const key of Object.keys(rulesObj)) {
      if (!RULE_KEY_SET.has(key)) {
        ...
        errors.push({
          ...
          severity: "error",
          suggestion: fallback ? `move to \`${fallback}\` until it lands as a first-class rule` : `valid rule keys: ${RULE_KEYS.join(", ")}`
        });
      }
    }
...
function checkToolAccessShadow(rules, warnings) {
  const ta = rules.tool_access;
  if (!ta || typeof ta !== "object" || Array.isArray(ta))
    return;
  const tao = ta;
  const allow = Array.isArray(tao.allow) ? tao.allow : [];
  const deny = Array.isArray(tao.deny) ? tao.deny : [];
  const allowSet = new Set(allow.filter((v) => typeof v === "string"));
  for (const d of deny) {
...
function checkEgressLeak(rules, warnings) {
  ...
  if (!eg || typeof eg !== "object" || Array.isArray(eg)) {
    ...
    if (hasPathRules) {
      warnings.push({
        path: "rules.egress",
        message: "policy defines filesystem rules but no egress policy; network calls default to kernel default",
        severity: "warning",
  ...
  const ego = eg;
  const allow = Array.isArray(ego.allow) ? ego.allow : [];
...
function checkDeadCapability(rules, warnings) {
...
async function listReceipts(daemon, opts = {}) {
  if (!daemon) {
    throw new NotInitializedError("receipts() requires daemon mode; construct via ChioBridge.fromDaemon({...})");
  }
  const client = daemon.receiptQueryClient();
  const params = {};
  if (opts.since)
    params.since = Math.floor(opts.since.getTime() / 1e3);
  if (opts.until)
    params.until = Math.floor(opts.until.getTime() / 1e3);
  if (opts.limit !== void 0)
    params.limit = opts.limit;
  if (opts.capabilityId)
    params.capabilityId = opts.capabilityId;
  if (opts.toolServer)
    params.toolServer = opts.toolServer;
  if (opts.toolName)
    params.toolName = opts.toolName;
  const res = await client.query(params);
  return res.receipts;
}
async function* streamReceipts(daemon, opts = {}) {
  if (!daemon) {
    throw new NotInitializedError("receiptStream() requires daemon mode; construct via ChioBridge.fromDaemon({...})");
  }
  const client = daemon.receiptQueryClient();
  const poll = opts.pollIntervalMs ?? 2e3;
  let since = opts.since ? Math.floor(opts.since.getTime() / 1e3) : Math.floor(Date.now() / 1e3);
...
function verifyReceiptValue(input) {
  const v = typeof input === "string" ? verifyReceiptJson(input) : verifyReceipt(input);
  return v.signature_valid && v.parameter_hash_valid;
}
async function exportEvidence(daemon, opts) {
  if (!daemon) {
    throw new NotInitializedError("exportEvidence() requires daemon mode; construct via ChioBridge.fromDaemon({...})");
  }
  const since = Math.floor(opts.since.getTime() / 1e3);
  const until = opts.until ? Math.floor(opts.until.getTime() / 1e3) : void 0;
  const body = { since };
  if (until !== void 0)
    body.until = until;
  const res = await daemon.trust("POST", "/v1/evidence/export", body);
  if (!res.ok) {
    throw new Error(`evidence export failed: HTTP ${res.status}: ${res.raw.slice(0, 300)}`);
  }
  await writeFile2(opts.outPath, JSON.stringify(res.data, null, 2), "utf8");
  return opts.outPath;
}
...
var ChioBridge = class _ChioBridge {
  ...
  cli;
  receiptDbPath;
  constructor(daemon, cli, receiptDbPath) {
    this.daemon = daemon;
    this.cli = cli;
    this.receiptDbPath = receiptDbPath;
  }
  static fromDaemon(opts) {
    return new _ChioBridge(new DaemonClient(opts), new ChioCli(), opts.receiptDbPath);
  }
  static fromCli(opts = {}) {
    return new _ChioBridge(void 0, new ChioCli(opts.chioBinary));
  }
  async bond(opts) {
    const policy = await loadPolicy(opts.policyPath);
    const lint = await lintPolicy(policy);
    if (lint.errors.length > 0) {
      throw new ChioBridgeError("policy_invalid", `policy at ${opts.policyPath} has ${lint.errors.length} error(s): ${lint.errors.map((e) => `${e.path}: ${e.message}`).join("; ")}`, lint);
    }
    const createOpts = {};
    if (opts.ttl)
      createOpts.ttl = opts.ttl;
   ...
   */
  async revoke(passportIdOrDid) {
    if (this.daemon) {
      const passportId = await this.resolvePassportId(passportIdOrDid);
      const res = await this.daemon.trust("POST", `/v1/passport/statuses/${encodeURIComponent(passportId)}/revoke`, {});
      if (!res.ok) {
        throw new ChioBridgeError("revoke_failed", `revoke failed: HTTP ${res.status}: ${res.raw.slice(0, 300)}`);
      }
      return;
    }
    if (this.cli) {
      ...
      if (result.exitCode !== 0) {
        throw new ChioBridgeError("revoke_failed", `chio passport status revoke failed (code ${result.exitCode}): ${result.stderr}`);
      }
      return;
    }
    throw new NotInitializedError("revoke() requires daemon or CLI client");
  }
  async resolvePassportId(input) {
    if (!isChioDid(input))
      return input;
    if (!this.daemon)
      return input;
    const res = await this.daemon.trust("GET", "/v1/passport/statuses");
    if (!res.ok || !res.data?.passports) {
      throw new ChioBridgeError("revoke_failed", `could not list passport statuses to resolve ${input}: HTTP ${res.status}`);
    }
    const matches = res.data.passports.filter((p) => p.subject === input);
    if (matches.length === 0) {
      throw new ChioBridgeError("revoke_failed", `no passport lifecycle record found for subject ${input}`);
    }
    matches.sort((a, b) => (b.publishedAt ?? 0) - (a.publishedAt ?? 0));
    const active = matches.find((p) => p.status === "active") ?? matches[0];
    if (!active) {
      throw new ChioBridgeError("revoke_failed", `no passport record to revoke for subject ${input}`);
    }
   ...
   * not passportId).
   */
  async status(passportId) {
    const toBondStatus = (d) => ({
      bonded: d.status === "active",
      status: d.status ?? "not-found",
      ttlExpiresAt: d.validUntil ?? ""
    });
    if (this.daemon) {
      const res = await this.daemon.trust("GET", `/v1/passport/statuses/${encodeURIComponent(passportId)}`);
      if (!res.ok) {
        throw new ChioBridgeError("status_failed", `status failed: HTTP ${res.status}`);
      }
      return toBondStatus(res.data);
    }
    if (this.cli) {
      const data = await this.cli.runJson([
        "passport",
        "status",
        "--id",
        passportId
   ...
   */
  async revokeAllForSubject(did) {
    if (!this.daemon) {
      throw new NotInitializedError("revokeAllForSubject() requires daemon mode; construct via ChioBridge.fromDaemon({...})");
    }
    if (!isChioDid(did)) {
      throw new ChioBridgeError("invalid_arg", `revokeAllForSubject() requires a did:chio/did:arc subject, got ${did}`);
    }
    const listRes = await this.daemon.trust("GET", "/v1/passport/statuses");
    if (!listRes.ok || !listRes.data?.passports) {
      throw new ChioBridgeError("revoke_failed", `could not list passport statuses for ${did}: HTTP ${listRes.status}`);
    }
    const matches = listRes.data.passports.filter((p) => p.subject === did && p.status === "active");
    const revokedPassportIds = [];
    const failed = [];
...
function deriveCapabilityScopeFromPolicy(policy, budgetUsd) {
  const rules = policy.rules;
  const toolAccess = rules?.tool_access ?? {};
  const allow = Array.isArray(toolAccess.allow) ? toolAccess.allow : [];
  const grants = [];
  const budgetAmount = typeof budgetUsd === "number" && Number.isFinite(budgetUsd) && budgetUsd > 0 ? { units: Math.floor(budgetUsd * 100), currency: "USD" } : void 0;
  for (const entry of allow) {
    if (typeof entry === "string") {
      ...
      if (budgetAmount)
        grant.max_total_cost = budgetAmount;
      grants.push(grant);
    } else if (entry && typeof entry === "object") {
      const obj = entry;
      const server_id = typeof obj.server_id === "string" ? obj.server_id : "*";
...
function buildBridge() {
  const token = process.env.CHIO_SERVICE_TOKEN ?? process.env.CLAUDE_PLUGIN_OPTION_SERVICE_TOKEN;
  const trustUrl = process.env.CHIO_TRUST_URL ?? process.env.CLAUDE_PLUGIN_OPTION_TRUST_URL;
  const mcpEdgeUrl = process.env.CHIO_MCP_EDGE_URL ?? process.env.CLAUDE_PLUGIN_OPTION_MCP_EDGE_URL;
  const chioBinary = process.env.CHIO_BINARY ?? process.env.CHIO_BIN ?? process.env.CLAUDE_PLUGIN_OPTION_CHIO_BINARY ?? "chio";
  if (token) {
...
function getPolicyPath() {
  return process.env.CHIO_POLICY_PATH ?? process.env.CLAUDE_PLUGIN_OPTION_POLICY_PATH ?? void 0;
}
...
function canonicalizeRawNumber(raw) {
  if (!/[.eE]/.test(raw)) {
    return raw === "-0" ? "0" : raw;
  }
  ...
  if (rendered === void 0) {
    throw new ChioInvariantError2("canonical_json", "canonical JSON does not support invalid numbers");
  }
  return rendered;
}
function canonicalizeParsedJson(value) {
  switch (value.type) {
    case "null":
      return "null";
    case "boolean":
    ...
    case "string":
      return canonicalizeString2(value.value);
    case "array":
      return `[${value.items.map((item) => canonicalizeParsedJson(item)).join(",")}]`;
    case "object":
      return `{${Array.from(value.entries.entries()).sort(([left], [right]) => compareUtf162(left, right)).map(([key, entryValue]) => `${canonicalizeString2(key)}:${canonicalizeParsedJson(entryValue)}`).join(",")}}`;
  }
}
var JsonTextCanonicalParser = class {
  input;
  position = 0;
  constructor(input) {
    this.input = input;
  ...
  parse() {
    ...
    if (this.position !== this.input.length) {
      throw new ChioInvariantError2("json", "input contains trailing data after JSON value");
    }
    return value;
  }
  parseValue() {
    ...
    switch (next) {
      ...
      default:
        ...
        throw new ChioInvariantError2("json", "input is not valid JSON");
    }
  }
  parseObject() {
    this.position += 1;
    ...
    while (true) {
      ...
      this.expect(":");
      if (entries.has(key)) {
        throw new ChioInvariantError2("json", `input contains duplicate object key: ${key}`);
      }
  ...
  parseString() {
    ...
    while (this.position < this.input.length) {
      ...
      if (current === '"') {
        this.position += 1;
        try {
          return JSON.parse(this.input.slice(start, this.position));
        } catch (cause) {
          throw new ChioInvariantError2("json", "input is not valid JSON", { cause });
        }
      }
      if (current === "\\") {
        this.position += 1;
...
function canonicalizeJsonString2(input) {
...
function hexToBuffer2(hex, expectedBytes, code) {
...
function createEd25519PrivateKey(seedHex) {
  try {
    return createPrivateKey3({
      key: Buffer.concat([ED25519_PKCS8_PREFIX2, hexToBuffer2(seedHex, 32, "invalid_hex")]),
      format: "der",
      type: "pkcs8"
    });
  } catch (cause) {
    ...
    throw new ChioInvariantError2("invalid_hex", "value is not a valid Ed25519 seed", { cause });
  }
}
function publicKeyHexFromSeedHex(seedHex) {
  const privateKey = createEd25519PrivateKey(seedHex);
  const publicKeyDer = createPublicKey3(privateKey).export({
    format: "der",
    type: "spki"
  });
  return Buffer.from(publicKeyDer).subarray(ED25519_SPKI_PREFIX2.length).toString("hex");
}
function signEd25519Message2(message, seedHex) {
  const privateKey = createEd25519PrivateKey(seedHex);
  ...
  return {
    ...
    signature_hex: Buffer.from(signMessage2(null, messageBuffer, privateKey)).toString("hex")
  };
}
...
function signJsonStringEd255192(input, seedHex) {
  const canonical_json = canonicalizeJsonString2(input);
  const signed = signEd25519Message2(canonical_json, seedHex);
  return {
    canonical_json,
...
async function approve(args) {
  ...
  const sig = signJsonStringEd255192(input, key.privateKeyHex);
  const bundle = {
...
async function postAuthority(bundle) {
  const token = process.env.CHIO_SERVICE_TOKEN ?? process.env.CLAUDE_PLUGIN_OPTION_SERVICE_TOKEN;
  if (!token) return;
  const base = (process.env.CHIO_TRUST_URL ?? process.env.CLAUDE_PLUGIN_OPTION_TRUST_URL ?? "http://127.0.0.1:8940").replace(/\/$/, "");
...
async function receiptLast() {
  ...
  let signatureValid = "unchecked";
  try {
    signatureValid = await bridge.verifyReceipt(receipt);
  } catch (err) {
    signatureValid = `error: ${err.message}`;
  }
  return JSON.stringify(
    {
      id: receipt.id,
